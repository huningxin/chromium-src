...
bool IsDepthwiseConv2d(const MLOperator* conv2d) {
  ...
  auto get_channel =
      [](const webnn::Dimension& dim) -> std::optional<uint32_t> {
    if (std::holds_alternative<uint32_t>(dim)) {
      return std::get<uint32_t>(dim);
    }
    return std::nullopt;
  };

  std::optional<uint32_t> maybe_input_channels, maybe_output_channels;

  switch (options->inputLayout().AsEnum()) {
    case blink::V8MLInputOperandLayout::Enum::kNchw:
      maybe_input_channels = get_channel(input_shape[1]);
      maybe_output_channels = get_channel(output_shape[1]);
      break;
    case blink::V8MLInputOperandLayout::Enum::kNhwc:
      maybe_input_channels = get_channel(input_shape[3]);
      maybe_output_channels = get_channel(output_shape[3]);
      break;
  }
  if (!maybe_input_channels || !maybe_output_channels) {
    return false;
  }
  input_channels = *maybe_input_channels;
  output_channels = *maybe_output_channels;
